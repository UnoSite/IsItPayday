...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data():
        ...
        try:
            # Log the configuration used for calculation
            _LOGGER.debug(
            ...
            next_payday = await async_calculate_next_payday(
                ...
                pay_frequency=data[CONF_PAY_FREQ],
                pay_day=data.get(CONF_PAY_DAY),
                last_pay_date=data.get(CONF_LAST_PAY_DATE),
                weekday=data.get(CONF_WEEKDAY),  # Bemærk: CONF_WEEKDAY bruges nu korrekt til weekly
                bank_offset=data.get(CONF_BANK_OFFSET, 0),
            )
